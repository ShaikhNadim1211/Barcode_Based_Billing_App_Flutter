...
class ShopWorkerAddProductScreen extends StatefulWidget
{
  final String ownerId;
  final String shopId;
  final String workerId;

  ShopWorkerAddProductScreen(this.ownerId, this.shopId, this.workerId);

  @override
  State<StatefulWidget> createState()
  {
    return ShopWorkerAddProductScreenState();
  }
}

class ShopWorkerAddProductScreenState extends State<ShopWorkerAddProductScreen>
{
  ...
  Color productNameMssgColor = Colors.red;
  Color buyingPriceMssgColor = Colors.red;
  Color sellingPriceMssgColor = Colors.red;
  Color quantityMssgColor = Colors.red;
  Color barcodeNumberMssgColor = Colors.red;
  ...
  bool productNameVerification = false;
  bool buyingPriceVerification = false;
  bool sellingPriceVerification = false;
  bool quantityVerification = false;
  bool barcodeNumberVerification = false;
  ...
  bool barcodeNumberVisibility = true;
  bool barcodeNumberIsChecked = false;
  ...
  Future<bool> _checkBarcodeNumberExists(String barcodeNumber) async
  {
    try
    {
      QuerySnapshot snapshot = await FirebaseFirestore.instance.collection("product")
          .where("ownerid", isEqualTo: this.username)
          .where("shopid", isEqualTo: this.shopName)
          .where("barcodenumber", isEqualTo: barcodeNumber).get();
  ...
  {
    super.initState();
    this.username = widget.ownerId.toString();
    this.shopName = widget.shopId;
              ...
              children: [
                ...
                inputFieldWidget.NormalInputFieldWithOnChange(
                    ...
                    IconButton(
                      ...
                      onPressed: () async{
                        String? scannedBarcode = await SimpleBarcodeScanner.scanBarcode(
                          context,
                        ...
                        if (scannedBarcode != null && scannedBarcode.isNotEmpty && scannedBarcode!="-1")
                        {
                          bool barcodeExists = await _checkBarcodeNumberExists(scannedBarcode);

                          if(barcodeExists)
                          {
                            barcodeNumberVerification = false;
                            barcodeNumberMssgColor = Colors.red;
                            barcodeNumberMssg = "Barcode already exists";
                            barcodeNumber.text = "";
                          }
                          else
                          {
                            barcodeNumberVerification = true;
                            barcodeNumberMssgColor = Colors.green;
                            barcodeNumberMssg = "";
                            barcodeNumber.text = scannedBarcode;
                          }
                          setState(() {
                          });
                        }
                        ...
                        (value) async
                    {
                      if(value.isEmpty)
                      {
                        barcodeNumberVerification = false;
                        barcodeNumberMssgColor = Colors.red;
                        barcodeNumberMssg = "Field is required";
                      }
                      else
                      {
                        ...
                        if(match)
                        {
                          bool barcodeExists = await _checkBarcodeNumberExists(value);

                          if(barcodeExists)
                          {
                            barcodeNumberVerification = false;
                            barcodeNumberMssgColor = Colors.red;
                            barcodeNumberMssg = "Barcode number already exists";
                          }
                          else
                          {
                            barcodeNumberVerification = true;
                        ...
                        else
                        {
                          barcodeNumberVerification = false;
                          barcodeNumberMssgColor = Colors.red;
                          barcodeNumberMssg = "Barcode number must be at least 5 digits";
                        }
                      }
                      setState(() {
                ...
                Row(
                  ...
                  children: [
                    Checkbox(
                      value: barcodeNumberIsChecked,
                      checkColor: Colors.green,
                      activeColor: Colors.white,

                      onChanged: (bool? newValue) async{
                        barcodeNumberIsChecked = newValue!;
                        if(barcodeNumberIsChecked)
                        {
                          barcodeNumberVisibility = false;
                          barcodeNumberVerification=true;
                          barcodeNumberMssgColor=Colors.green;
                          barcodeNumberMssg="";
                          String number = "";
                          while(true)
                          {
                            number = barcodeNumberGenerator.generateBarcode();
                            bool barcodeExists = await _checkBarcodeNumberExists(number);
                            if(barcodeExists)
                            {
                              continue;
                            }
                          ...
                          && this.barcodeNumberVerification && this.quantityVerification && optionalFieldVerification)
                      {
                        ...
                        else
                        {
                          ...
                          else
                          {
                            ...
                            try
                            {
                              String id = documentIdManage.getProductId();
                              ...
                              this.manufacturingDate.text = "";
                              this.expiringDate.text = "";

                              this.productNameMssg = "";
                              this.buyingPriceMssg = "";
                              this.sellingPriceMssg = "";
                              this.quantityMssg = "";
                              this.barcodeNumberMssg = "";
                              this.discountMssg = "";
                              this.sizeMssg = "";
                              this.colourMssg = "";
                              this.gstMssg = "";
                              ...
                              this.manufacturingDateMssg = "";
                              this.expiringDateMssg = "";

                              this.productNameMssgColor = Colors.red;
                              this.buyingPriceMssgColor = Colors.red;
                              this.sellingPriceMssgColor = Colors.red;
                              this.quantityMssgColor = Colors.red;
                              this.barcodeNumberMssgColor = Colors.red;
                              this.discountMssgColor = Colors.red;
                              this.sizeMssgColor = Colors.red;
                              this.colourMssgColor = Colors.red;
                              this.gstMssgColor = Colors.red;
                              this.categoryMssgColor = Colors.red;
                              this.descriptionMssgColor = Colors.red;
                              this.manufacturingDateMssgColor = Colors.red;
                              this.expiringDateMssgColor = Colors.red;

                              this.productNameVerification = false;
                              this.buyingPriceVerification = false;
                              this.sellingPriceVerification = false;
                              this.quantityVerification = false;
                              this.barcodeNumberVerification = false;
                              this.discountVerification = false;
                              this.sizeVerification = false;
                              this.colourVerification = false;
                              this.gstVerification = false;
                              this.categoryVerification = false;
                              this.descriptionVerification = false;
                              this.manufacturingDateVerification = false;
                              this.expiringDateVerification = false;

                              this.barcodeNumberVisibility = true;
                              this.barcodeNumberIsChecked = false;

                              this.manufacturingIntialDate = DateTime.now().subtract(Duration(days: 1));
                              this.manufacturingFirstdate = DateTime.now().subtract(Duration(days: 365 * 20));
                              this.manufacturingLastDate = DateTime.now().subtract(Duration(days: 1));

                              this.expiringIntialDate = DateTime.now().add(Duration(days: 1));
                              this.expiringFirstdate = DateTime.now().add(Duration(days: 1));
                              this.expiringLastDate = DateTime.now().add(Duration(days: 365 * 20));
